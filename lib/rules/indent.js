    ...
    create(context) {
        ...
        */
        function addElementListIndent(tokens, elements, offset) {
            ...
            if (offset === "first" && elements.length) {
                ignoreToken(sourceCode.getFirstToken(elements[0]));
            }
            ...
            elements.slice(1).forEach((element, index) => {
                 ...
                 */
                if (offset === "first") {
                    setDesiredOffset(sourceCode.getFirstToken(element), null, sourceCode.getFirstToken(elements[0]).loc.start.column / indentSize);
                } else {
        ...
        */
        function addFunctionParamsIndent(node, paramsIndent) {
            ...
            const paramTokens = getTokensAndComments(node).filter(token => token.range[0] >= openingParen.range[0]);
        ...
        */
        function addFunctionCallIndent(node) {
            ...
            // FIXME: don't use 1 here (same as the return index; issue)
            addElementListIndent(tokens, node.arguments, typeof options.CallExpression.arguments === "number" || typeof options.CallExpression.arguments === "string" ? options.CallExpression.arguments : 1);
        ...
        */
        ...
        function debugToken(token) {
            ...
            for (let currentToken = token; currentToken; currentToken = desiredOffsets.get(currentToken).from) {
                console.log(desiredOffsets.get(currentToken));
            }

            console.log('total desired indent: ', getDesiredIndent(token));
            console.log('total actual indent: ', getTokenIndent(token).goodChar / indentSize);
        }
        ...
        return {
            ...
            TemplateLiteral(node) {
                ...
                node.quasis.slice(1).forEach(quasi => {
