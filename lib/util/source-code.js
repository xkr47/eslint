...
SourceCode.prototype = {
    ...
    getComments(node) {
        ...
        if (node.type === "Program") {
            ...
            // Do not return comments that should belong to parent node
            let currentToken = this.getTokenOrCommentBefore(node);
            while (currentToken && isComment(currentToken) && currentToken.start >= node.parent.start) {
            ...
            while (currentToken && isComment(currentToken) && currentToken.end <= node.parent.end) {
        ...
        switch (node.type) {
            ...
            case "FunctionDeclaration":
                if (looksLikeExport(parent)) {
                    const parentComments = this.getComments(parent);
                    return findJSDocComment(parentComments.leading, parent.loc.start.line);
                }
                return findJSDocComment(comments.leading, node.loc.start.line);

            case "ClassExpression":
                const grandparentComments = this.getComments(parent.parent);
                return findJSDocComment(grandparentComments.leading, parent.parent.loc.start.line);

            case "ArrowFunctionExpression":
            case "FunctionExpression":

                if (parent.type !== "CallExpression" && parent.type !== "NewExpression") {
                    let parentComments = this.getComments(parent);

                    while (parent && !parentComments.leading.length && !/Function/.test(parent.type) && parent.type !== "MethodDefinition" && parent.type !== "Property") {
                        parent = parent.parent;
                        parentComments = this.getComments(parent);
                    }

                    return parent && (parent.type !== "FunctionDeclaration") ? findJSDocComment(parentComments.leading, parent.loc.start.line) : null;
                } else if (comments.leading.length) {
                    return findJSDocComment(comments.leading, node.loc.start.line);
                }
